...
const TopSange = () => {
    ...
    const top10Sange = [
      ...
      "2. 'Beatufil Things' - Benson Boone",
      "3. 'Birds Of a Feather' - Bilie Ellish",
      "4. 'Gata Only' - Floyymenor",
      "5. 'Lose Control' - Teddy Swims",
      "6. 'End of Beginning' - DJO",
      "7. 'Too Sweet' - Hozier",
      "8. 'One Of The Girls' - The Weeknd, Lilly Rose Deep",
    ...
    <span className="relative z-10">Vis svar</span>
        ...
        {showList && (
          <div className="grid grid-cols-2 gap-5 w-full m-auto ">
            {/* Venstre kolonne: 1-5 */}
            <ul>
              {top10Sange.slice(0, 5).map((film, index) => (
                <motion.li
                  key={index}
                  custom={index}
                  initial="hidden"
            ...
            {/* Højre kolonne: 6-10 */}
            <ul>
              {top10Sange.slice(5).map((film, index) => (
                <motion.li
                  key={index + 5}
                  custom={index + 5}
                  initial="hidden"
